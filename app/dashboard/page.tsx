...
//Ketentuan Paket
const PACKAGE_DETAILS: Record<number, { name: string; price: string; color: string; desc: string }> = {
    1: {
        name: "Basic Plan",
        price: "Rp 750.000",
...
export default function DashboardPage() {
    ...
    // Membaca data dari Smart Contract: "getUserPackages"
            ...
            <div className="max-w-5xl mx-auto">
                ...
                {isLoading && (
                    <div className="flex flex-col items-center justify-center py-20">
                        ...
                        <p className="text-gray-500">Mengambil data dari Blockchain...</p>
                    </div>
                )}
                ...
                {isError && (
                    <div className="bg-red-50 border border-red-200 p-6 rounded-xl text-center">
                        <p className="text-red-600">Terjadi kesalahan saat mengambil data paket.</p>
                    </div>
                )}

                {/* EMPTY STATE (User belum beli apa-apa) */}
                {!isLoading && userPackages && (userPackages as bigint[]).length === 0 && (
                    <div className="bg-white rounded-3xl p-12 text-center shadow-sm border border-gray-100">
                        <Package className="w-20 h-20 text-gray-300 mx-auto mb-6" />
                        <h3 className="text-xl font-bold text-gray-900 mb-2">Belum Ada Paket Aktif</h3>
                        <p className="text-gray-500 mb-8 max-w-md mx-auto">
                            Sepertinya Anda belum membeli layanan perlindungan hama apapun. Lindungi rumah Anda sekarang.
                        </p>
                        <Link
                            href="/pricing"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {userPackages && (userPackages as bigint[]).map((idBigInt, index) => {
                        ...
                        return (
                            <div key={index} className="bg-white rounded-2xl p-6 shadow-md hover:shadow-lg transition-all border border-gray-100 flex flex-col">
                                <div className="flex justify-between items-start mb-4">
                                    <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${details.color}`}>
                                        Aktif
                                    </div>
                                ...
                                <p className="text-sm text-gray-500 mb-4">{details.desc}</p>
